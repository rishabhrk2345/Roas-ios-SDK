...
enum DeviceContext {
    ...
    /// The shipped `PrivacyInfo.xcprivacy`, for the test that keeps it honest.
    /// `Bundle.module` is generated by SwiftPM only, so under CocoaPods (where
    /// the file ships as a resource bundle instead) this is nil rather than a
    /// compile error.
    static func privacyManifestURL() -> URL? {
        #if SWIFT_PACKAGE
        return Bundle.module.url(forResource: "PrivacyInfo", withExtension: "xcprivacy")
        #else
        return nil
        #endif
    }
    ...
    /// one.
    ///
    /// 0.1.10 is the App Store submission release: a `PrivacyInfo.xcprivacy`
    /// ships in the package (without one, every customer upload is refused
    /// with ITMS-91053 for API calls that are ours), `uptime_seconds` is no
    /// longer collected (no approved reason lets system boot time leave the
    /// device), and SKAdNetwork registration runs once at install instead of
    /// resetting the conversion value to 0 on every launch.
    static let sdkVersion = "0.1.10"
    ...
    // `uptime_seconds` is deliberately NOT collected on iOS (it was, through
    // 0.1.9, via `ProcessInfo.systemUptime`). That call is a "required reason"
    // API -- system boot time -- and every reason Apple accepts for it
    // (35F9.1, 8FFB.1, 3D61.1) forbids sending the value, or anything derived
    // from it, off the device. The field's whole purpose was to leave the
    // device as a device-farm signal, so there is no honest declaration that
    // covers it; a manifest claiming 35F9.1 while the beacon carried the value
    // would be false, and App Review reads the binary. Android keeps the
    // column (`SystemClock.elapsedRealtime` has no such rule). The server
    // already treats it as optional -- an absent key is simply not written.
    ...
    /// network and location move over a process lifetime and are read
    ...
    /// can attribute it. Best-effort. Call ONCE, on the install launch: the
    /// iOS 16.1+ form is a conversion-value update of 0, and SKAdNetwork 4
    /// honours a decrease, so a later call overwrites a real value the app has
    /// since reported (`Roas.updateConversionValue`). `Roas.configure` gates it
    /// on `installReported`.
